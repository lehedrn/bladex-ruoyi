	...
	public void produce(Report report, OutputStream outputStream) {
		...
		try {
			...
			if (orientation.equals(Orientation.landscape)) {
				...
				pageSize.setH(DxaUtils.points2dxa(paper.getWidth()));
				pageSize.setW(DxaUtils.points2dxa(paper.getHeight()));
			} else {
				pageSize.setOrient(STPageOrientation.PORTRAIT);
				pageSize.setW(DxaUtils.points2dxa(paper.getWidth()));
				pageSize.setH(DxaUtils.points2dxa(paper.getHeight()));
			}
			...
			pageMar.setLeft(DxaUtils.points2dxa(paper.getLeftMargin()));
			pageMar.setRight(DxaUtils.points2dxa(paper.getRightMargin()));
			pageMar.setTop(DxaUtils.points2dxa(paper.getTopMargin()));
			pageMar.setBottom(DxaUtils.points2dxa(paper.getBottomMargin()));
			...
			for (Page page : pages) {
				...
				table.getCTTbl().addNewTblPr().addNewTblW().setW(DxaUtils.points2dxa(tableWidth));
				for (int rowNumber = 0; rowNumber < rows.size(); rowNumber++) {
					...
					for (Column col : columns) {
						...
						tableCell.getCTTc().addNewTcPr().addNewTcW().setW(DxaUtils.points2dxa(width));
