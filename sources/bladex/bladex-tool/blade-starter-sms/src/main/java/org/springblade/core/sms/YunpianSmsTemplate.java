...
public class YunpianSmsTemplate implements SmsTemplate {
	...
	public SmsResponse sendMessage(SmsData smsData, Collection<String> phones) {
		...
		if (result.getCode() == Code.OK) {
			return new SmsResponse(Boolean.TRUE, result.getCode(), result.toString());
		} else {
			log.warn("短信发送失败，phones:{}，response:{}", phones, result);
			return new SmsResponse(Boolean.FALSE, result.getCode(), result.toString());
		}
	}
