	...
	public SmsResponse sendMessage(SmsData smsData, Collection<String> phones) {
		...
		try {
			...
			if (response.getHttpStatus() == SUCCESS && code.equalsIgnoreCase(OK)) {
				return new SmsResponse(Boolean.TRUE, response.getHttpStatus(), response.getData());
			} else {
				log.warn("短信发送失败，phones:{}，code:{}, response:{}", phones, code, response.getData());
				return new SmsResponse(Boolean.FALSE, response.getHttpStatus(), response.getData());
			}
