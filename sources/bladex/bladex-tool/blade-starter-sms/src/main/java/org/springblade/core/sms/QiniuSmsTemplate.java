...
public class QiniuSmsTemplate implements SmsTemplate {
	...
	public SmsResponse sendMessage(SmsData smsData, Collection<String> phones) {
		try {
			...
			if (response.isOK()) {
				return new SmsResponse(Boolean.TRUE, response.statusCode, response.toString());
			} else {
				log.warn("短信发送失败，phones:{}，response:{}", phones, response);
				return new SmsResponse(Boolean.FALSE, response.statusCode, response.toString());
			}
