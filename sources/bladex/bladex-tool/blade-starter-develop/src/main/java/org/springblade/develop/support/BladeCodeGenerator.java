...
public class BladeCodeGenerator {
	 ...
	 * 模版类型
	 */
	private String templateType = TEMPLATE_CRUD;
	/**
	 * 作者信息
	 */
	 ...
	 * 是否包含菜单SQL
	 */
	private Boolean hasMenuSql = Boolean.TRUE;
	/**
	 * 基础业务字段
	 */
	 ...
	 * 表单设计
	 */
	private String modelFormOption;
	/**
	 * 数据模型
	 */
	...
	private Map<String, Object> getCustomMap(String generateType) {
		...
		this.author = StringUtil.isBlank(author) ? AUTHOR_NAME : author;
		...
		customMap.put("modelFormOption", modelFormOption);
		...
		List<String> propertyImport = prototypeList.stream().filter(prototype -> {
			...
			.map(prototype -> String.valueOf(prototype.get("propertyEntity")))
			.filter(entity -> entity != null && !"null".equals(entity))
	...
	private Map<String, String> getCustomFile(String type) {
		...
		if (Func.isNotBlank(packageWebDir)) {
			if (Func.equals(codeStyle, SWORD_NAME)) {
				...
				if (Func.isNotEmpty(this.modelFormOption)) {
					customFile.put("crud.vue", "/templates/saber3/crud/crud-form.vue.btl");
					customFile.put("option.js", "/templates/saber3/crud/option-form.js.btl");
				}
	...
	private FastAutoGenerator getAutoGenerator(Map<String, Object> customMap, Map<String, String> customFile) {
		...
		String url = getJdbcUrl(Func.toStr(this.url, props.getProperty("spring.datasource.url")));
		...
		return FastAutoGenerator.create(url, username, password)
			.globalConfig(builder -> builder.author(author).dateType(DateType.TIME_PACK).enableSwagger().outputDir(getOutputDir()).disableOpenDir())
	...
	private String getJdbcUrl(String url) {
		...
		if (dbType == DbType.MYSQL) {
			url += "&allowLoadLocalInfile=false&allowUrlInLocalInfile=false&allowLoadLocalInfileInPath=false";
		}
		return url;
	}
