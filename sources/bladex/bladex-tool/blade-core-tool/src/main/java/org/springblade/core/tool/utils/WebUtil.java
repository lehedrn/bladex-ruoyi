...
public class WebUtil extends org.springframework.web.util.WebUtils {
	...
	public static String getRequestBody(ServletInputStream servletInputStream) {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(servletInputStream, StandardCharsets.UTF_8))) {
			...
			// 记录日志或者重新抛出异常
			log.warn("WebUtil获取RequestBody失败：", e);
		}
		return sb.toString();
	}
	...
	public static String getRequestContent(HttpServletRequest request) {
		try {
			...
			if (StringUtil.isNotBlank(queryString)) {
				return URLDecoder.decode(queryString, StandardCharsets.UTF_8).replaceAll("&amp;", "&").replaceAll("%22", "\"");
			}
			// 获取请求体
			...
			// 记录日志或者重新抛出异常
			log.warn("WebUtil获取RequestContent失败：", e);
			return StringPool.EMPTY;
		}
	}
