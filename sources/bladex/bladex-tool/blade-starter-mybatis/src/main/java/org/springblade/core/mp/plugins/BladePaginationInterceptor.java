...
import org.springblade.core.mp.support.SQLServerOrderDialect;
...
public class BladePaginationInterceptor extends PaginationInnerInterceptor {
	...
	protected IDialect autoDialect(Executor executor) {
		...
		if (url.contains(":yasdb:")) {
			// 指定 YashanDB 方言为 Oracle
			return new OracleDialect();
		} else if (url.contains(":sqlserver:") || url.contains(":microsoft:")) {
			// 指定自定义 SQLServer 方言，否则不加 order by 的分页将报错
			// https://github.com/baomidou/mybatis-plus/issues/3969
			return new SQLServerOrderDialect();
		} else {
