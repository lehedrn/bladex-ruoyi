...
public class CacheUtil {
	 ...
	 */
	@Nullable
	public static Object get(String cacheName, String keyPrefix, Object key, String tenantId) {
		if (Func.hasEmpty(cacheName, keyPrefix, key)) {
			return null;
		}
		Cache.ValueWrapper valueWrapper = getCache(cacheName, tenantId).get(keyPrefix.concat(String.valueOf(key)));
		return valueWrapper == null ? null : valueWrapper.get();
	}
	 ...
	 */
	@Nullable
	public static <T> T get(String cacheName, String keyPrefix, Object key, @Nullable Class<T> type, String tenantId) {
		if (Func.hasEmpty(cacheName, keyPrefix, key)) {
			return null;
		}
		return getCache(cacheName, tenantId).get(keyPrefix.concat(String.valueOf(key)), type);
	}
	...
	public static <T> T get(String cacheName, String keyPrefix, Object key, Callable<T> valueLoader, Boolean tenantMode) {
		...
		Cache cache = getCache(cacheName, tenantMode);
		return getWithLoader(cache, keyPrefix, key, valueLoader);
	}
	...
	public static <T> T get(String cacheName, String keyPrefix, Object key, Callable<T> valueLoader, String tenantId) {
		...
		Cache cache = getCache(cacheName, tenantId);
		return getWithLoader(cache, keyPrefix, key, valueLoader);
	}
	 ...
	 */
	@Nullable
	private static <T> T getWithLoader(Cache cache, String keyPrefix, Object key, Callable<T> valueLoader) {
	 ...
	 * @param tenantId  租户ID
	 */
	public static void put(String cacheName, String keyPrefix, Object key, @Nullable Object value, String tenantId) {
		getCache(cacheName, tenantId).put(keyPrefix.concat(String.valueOf(key)), value);
	}
